...
export async function getJobs(filters?: JobFilters): Promise<Job[]> {
  ...
  const url = getApiUrl(`/jobs${queryString ? `?${queryString}` : ''}`);
  return fetchWithAuth(url);
}

export async function getJob(id: string): Promise<Job> {
  return fetchWithAuth(getApiUrl(`/jobs/${id}`));
}

export async function createJob(data: CreateJobDto): Promise<Job> {
  return fetchWithAuth(getApiUrl('/jobs'), {
    method: 'POST',
...
export async function updateJobStatus(
  ...
  return fetchWithAuth(getApiUrl(`/jobs/${id}/status`), {
    method: 'PATCH',
    body: JSON.stringify(data),
  });
}
...
export async function startJob(id: string, data?: StartJobDto): Promise<Job> {
  return fetchWithAuth(getApiUrl(`/jobs/${id}/start`), {
    method: 'POST',
    body: JSON.stringify(data || {}),
  });
}
...
export async function completeJob(id: string, data: CompleteJobDto): Promise<Job> {
  return fetchWithAuth(getApiUrl(`/jobs/${id}/complete`), {
    method: 'POST',
    body: JSON.stringify(data),
  });
}
...
export async function addJobNotes(id: string, data: AddJobNoteDto): Promise<Job> {
  return fetchWithAuth(getApiUrl(`/jobs/${id}/notes`), {
    method: 'PATCH',
...
export async function uploadJobPhoto(
  ...
  const response = await fetch(getApiUrl(`/jobs/${id}/photos`), {
    method: 'POST',
    body: formData,
  });
...
export async function deleteJobPhoto(jobId: string, photoId: string): Promise<void> {
  return fetchWithAuth(getApiUrl(`/jobs/${jobId}/photos/${photoId}`), {
    method: 'DELETE',
  });
}
